...
public class WebServiceClientMessage {
    ...
    private static boolean isTokenValid(Context context) {
        ...
        if (sharedSettings.contains(
                ...
                sharedSettings.contains(context.getString(R.string.pref_ws_msg_access_token_expiration))) {

            expirationTime = sharedSettings.getLong(context.getString(R.string.pref_ws_msg_access_token_expiration), 0);

            if (Calendar.getInstance().getTime().getTime() < expirationTime) {
                accessToken = new AccessToken();
                accessToken.setAccess_token(sharedSettings.getString(context.getString(R.string.pref_ws_msg_access_token), ""));
